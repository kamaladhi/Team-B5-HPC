...
class OpenMPImageProcessor {
    ...
    public:
        void processGaussianFilter(const cv::Mat& input, const std::string& outputPath, int numThreads) { // input original image , output basepath to save the result image
            ...
            performance.startTimer(); // starts the timer how long does the performance goes 

            GaussianFilterOMP gFilter;
            cv::Mat result = gFilter.apply(input, 5, 1.5);  // giving kernal size as 5 x 5
        ...
        void processCustomConvolution(const cv::Mat& input, const std::string& outputPath, int numThreads) {
            ...
            cv::Mat result;
            // if number of channel is 1 then we can perform convo2D
            if (input.channels() == 1) { 
                result = ConvolutionEngineOMP::convolve2D(input, edgeKernel);
            } else {                                                // If not split them into three invidial channels R B G and apply convolution for each
                ...
                for (int c = 0; c < input.channels(); c++) {
                    outChannels[c] = ConvolutionEngineOMP::convolve2D(channels[c], edgeKernel); // Each color channel is processed in parallel
                }
                cv::merge(outChannels, result); // then merge the channel
            }

            performance.stopTimer();

            cv::Mat finalResult;
            result.convertTo(finalResult, CV_8U); // convert the final output into 8-bit unsigned int

            performance.printResults("Custom Kernal Convolution (OpenMP)", numThreads);
    ...
    // convolution on the raw array of floats without using open cv
        void processRawConvolution(const cv::Mat& input, const std::string& outputPath, int numThreads) {
            ...
            cv::Mat grayInput; 
            // if input is color image or color channel then convert into grey scale image 
            if (input.channels() == 3) {
            cv::cvtColor(input, grayInput, cv::COLOR_BGR2GRAY);
            } else {
            grayInput = input.clone(); // Raw convolution here is applied only on a single channel.
            }
            // allocating a raw array based on the input rows and columns.
            float* inputData = new float[grayInput.rows * grayInput.cols];
            float* outputData = new float[grayInput.rows * grayInput.cols];

            // Copy input data
            #pragma omp parallel for collapse(2) // collapse (2) treats both the loop as single loop 
            for (int y = 0; y < grayInput.rows; y++) {
                for (int x = 0; x < grayInput.cols; x++) {
                    inputData[y * grayInput.cols + x] = static_cast<float>(grayInput.at<uchar>(y, x)); //Copies grayscale pixel values into the 1D float array inputData.
                }
            }
            // averaging kernel or box blur kernal avearge of all value is equal to 1 
            //Produces blurred / smoothed images.
            float blurKernel[9] = {
            0.111f, 0.111f, 0.111f,  // 1/9 ≈ 0.111 ensures that the overall brightness of the image doesn’t change.
